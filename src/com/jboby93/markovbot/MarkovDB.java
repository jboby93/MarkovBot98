...
public class MarkovDB {
	...
	public int getDBSize() { return _data.size(); }
	
	private int n = 2; public int getN() { return n; }
	
	public MarkovDB() { this(2); }
	...
	public String getNextWord() {
		...
		try {
			...
			for(int i = 0; i < n-1; i++) { newKey[i] = newKey[i+1]; }
	...
	public void teach(String source) {
		...
		for(int i = 0; i < words.length - n - 1; i++) {
			...
			for(int j = 1; j < n; j++) key = key + " " + sanitize(words[i+j]);
	...
	private void readFromFile(String file, boolean append) throws IOException {
		...
		for(String s : lines) {
			...
			if(App.Left(s,  2).equals("N=")) {
				...
				for(String v : value) { value_list.add(v); }
	...
	public void save() {
		if(filename.equals("null")) { saveAs(); } else {
			try {
	...
	private void writeToFile(String file) throws IOException {		
		...
		for(Map.Entry<String, ArrayList<String>> pair : _data.entrySet()) {
			...
			for(Object s : value_arr) {
				...
				//is there more?
				if(i + 1 < pair.getValue().size()) value_sb.append(choice_sep);
			...
			for(String t : terms) {
				...
				for(String o : entry.getValue()) {
					if(o.toLowerCase().equals(t)) {
						...
						if(t.length() > 1) thisResult.score++;
					}
				}
	...
	private String getKeyForIndex(int index) { return (String)_data.keySet().toArray()[index]; }
